...
missing_value= train.isnull().sum(axis=0).reset_index()
missing_value.columns=['column_name', 'missing_count']
missing_value['missing_ratio']=missing_value['missing_count'] / train.shape[0]
...
rects=ax.barh(ind, missing_value.missing_count.values, color='red')
...
missing_value= test.isnull().sum(axis=0).reset_index()
missing_value.columns=['column_name', 'missing_count']
missing_value['missing_ratio']=missing_value['missing_count'] / train.shape[0]
missing_value.ix[missing_value['missing_ratio']>0.99]


# no missing value in test and train data is clean
...
dtype1=train.dtypes.reset_index()
dtype1.columns=["Count","Column Types"]
dtype1
...
dtype1=test.dtypes.reset_index()
dtype1.columns=["Count","Column Types"]
dtype1
...
plt.subplot(211)
wc=Wordcloud(background_color="black",max_words=10000,mask=hcmask,stopwords=STOPWORD,max_fount_size=40)
wc.generate(" ".join(eap))
plt.title("Edger Allen Poe (The Raven)")
plt.title(wc.recolor(colormap=PuBu,random_state=17),alpha=0.9)
plt.axis('off')
...
firstcloud = WordCloud(
                          ...
                          background_color='black',
                          width=2500,
                          hei## Number of words in the text ##
...
test["mean_word_len"] = test["text"].apply(lambda x: np.mean([len(w) for w in str(x).split()]))ght=1800
...
X_train, X_test, y_train, y_test = train_test_split(train, test, test_size=0.2, random_state=21)

print('X_train size: {}'.format(X_train.shape))
print('X_test size: {}'.format(X_test.shape))
print('y_train size: {}'.format(y_train.shape))
print('y_test size: {}'.format(y_test.shape))    title='Confusion matrix of XGB, without normalization')
plt.show()
